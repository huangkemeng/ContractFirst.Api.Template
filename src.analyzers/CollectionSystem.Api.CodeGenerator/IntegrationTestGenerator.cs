...
[Generator]
public class IntegrationTestGenerator : ISourceGenerator
{
        ...
        try
        {
...

            {
                ...
                foreach (var typeSymbol in typeSymbols)
                {
                    ...
                    {
                        "Autofac",
                        "CollectionSystem.Api.Primary.Contracts.Bases",
                        "Xunit"
                    };
                    var contextType = "";
                    var sendString = "";
    ...
    private static List<ITypeSymbol> GetTypeSymbols(ISymbol symbol)
    {
        ...
        else if (symbol is ITypeSymbol typeSymbol)
        {
            if (typeSymbol.AllInterfaces.Any(e =>
                    e.ToDisplayString().Contains("CollectionSystem.Api.Primary.Contracts.Bases.ITestable")))
            {
        ...
        string sendString)
    {
        return
            $@"
{usingString}
namespace CollectionSystem.Api.IntegrationTests.Generated;
...
{{
    ...
    {{
        {contextString}
        var handler = TestServices.Resolve<{handlerName}>();
        if (handler != null)
        {{
            ...
            {{
                ...
                {{
                    ...
                    {{
                        if (testCase.Build != null)
                        {{
                            Build(testCase.Build);
                            context.Container = TestServices;
                            context.Mediator = TestMediator;
                            context.DbContext = TestDbContext;
                        }}

                        if (testCase.DatabaseCleanupRequired)
                        {{
