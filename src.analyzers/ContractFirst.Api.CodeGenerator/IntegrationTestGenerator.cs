...
public class IntegrationTestGenerator : ISourceGenerator
{
        ...
        try
        {
...

            {
                ...
                foreach (var typeSymbol in typeSymbols)
                {
                    ...
                    {
                        "Autofac",
                        "ContractFirst.Api.Primary.Contracts.Bases",
                        "Xunit"
                    };
                    var contextType = "";
                    var sendString = "";
    ...
    private static List<ITypeSymbol> GetTypeSymbols(ISymbol symbol)
    {
        ...
        else if (symbol is ITypeSymbol typeSymbol)
        {
            if (typeSymbol.AllInterfaces.Any(e =>
                    e.ToDisplayString().Contains("ContractFirst.Api.Primary.Contracts.Bases.ITestable")))
            {
        ...
        string sendString)
    {
        return
            $@"
{usingString}
namespace ContractFirst.Api.IntegrationTests.Generated;
