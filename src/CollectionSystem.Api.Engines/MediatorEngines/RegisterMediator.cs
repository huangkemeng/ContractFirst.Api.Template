...
public class RegisterMediator : IBuilderEngine
{
    ...
    private static Type? GetContractInputAndOutputType(Type contractType)
    {
        return contractType.GetInterfaces().FirstOrDefault(e =>
            e.IsGenericType &&
            new[] { typeof(ICommandHandler<>), typeof(ICommandHandler<,>), typeof(IRequestHandler<,>) }.Contains(
                e.GetGenericTypeDefinition()));
    }
    ...
    private static SyntaxTree? GetSyntaxTree(Type contractType)
    {
        ...
        if (iHandlerType != null)
        {
            ...
            root = root.AddUsings(SyntaxFactory.UsingDirective(SyntaxFactory.ParseName(typeof(IContract<>).Namespace!)))
                ...
                .AddUsings(
                    SyntaxFactory.UsingDirective(SyntaxFactory.ParseName(typeof(AbstractValidator<>).Namespace!)))
            ...
            var handleMethodDeclaration = SyntaxFactory.MethodDeclaration(SyntaxFactory.ParseTypeName(returnTypeString),
            ...
            var testMethodDeclaration = SyntaxFactory.MethodDeclaration(SyntaxFactory.ParseTypeName(nameof(Task)),
                    nameof(IContract<IMessage>.TestAsync))
                .AddModifiers(SyntaxFactory.Token(SyntaxKind.PublicKeyword))
                .WithBody(SyntaxFactory.Block(
            ...
            var validatorMethodDeclaration = SyntaxFactory.MethodDeclaration(
                    ...
                    nameof(IContract<IMessage>.Validator))
                .AddModifiers(SyntaxFactory.Token(SyntaxKind.PublicKeyword))
            ...
            var namespaceDeclaration = SyntaxFactory.NamespaceDeclaration(SyntaxFactory.ParseName(
                !string.IsNullOrWhiteSpace(contractType.Namespace)
